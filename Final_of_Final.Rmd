...
```{r}
us_mutate <- us_youtube_trend %>%
  mutate(new= ((comment_count/views)*1000))

us_mutate_categ <-aggregate(views~category_id, data = us_mutate, sum)%>%

us_mutate_categ
#코멘트 나누기 뷰를 뮤테이트 해서 컬럼 만들고 카테고리 탑 10해서 일정 뷰당 코멘트 많은 수 탑 파이브) —> 디스라이크 
